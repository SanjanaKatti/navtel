      ...
      <main className="pt-20">
        ...
        <section className="py-24 bg-[#F8FAFC] overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-[1fr_1.5fr_1fr] gap-12 items-center">
              ...
              <div className="space-y-8 ml-10">
                ...
                <button 
                  ...
                  className="bg-brand-primary text-white px-10 py-4 rounded-full font-bold text-lg hover:bg-brand-deep transition-all shadow-lg shadow-brand-primary/20 transform hover:-translate-y-1"
                >
                  Explore
                </button>
              </div>
                ...
                <div className="w-full max-w-lg aspect-[1.4/1] relative group/hardware">
                  ...
                  <div className="lg:text-right">
                    <h2 className="text-display mr-10">
                      <span className="text-brand-primary">Basic</span><br/>
                      Models
                    </h2>
                  </div>
                </div>
            ...
            {/* KEY FEATURES */}
            <section className="py-16 md:py-20 bg-white rounded-t-[3rem] shadow-2xl relative z-10 -mt-8">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                  <h2 className="text-h1 mb-3">
                    Track your fleet. <span className="text-brand-primary">Monitor your fuel.</span>
                  </h2>
                  <p className="text-body max-w-2xl mx-auto">
                    Essential tools for GPS tracking, fuel control, and fleet automation.
              </p>
            </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {[
                    ...
                    <div key={idx} className="group p-8 md:p-10 bg-white rounded-[2.5rem] border border-gray-100 hover:shadow-2xl transition-all duration-500 relative overflow-hidden">
                      ...
                      <div className="pt-4">
                        ...
                        <p className="text-caption text-brand-primary mb-3">{feature.sub}</p>
              ...
              {filteredDevices.map((device, idx) => (
                  ...
                  key={idx}
                      className={`bg-white flex flex-col border rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden group relative ${
                        selectedForCompare.includes(device.name) ? 'border-brand-primary ring-2 ring-brand-primary/20' : 'border-gray-100'
                      }`}
                    >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {[
                    { title: "Reliable Updates", desc: "New features and improvements, delivered with stability in mind." },
                    { title: "Easy USB Configuration", desc: "Configure the device directly via USB — no external power supply required." },
                    { title: "Expert Support", desc: "Fast, clear, and helpful assistance — from people who know the product inside and out." },
                    { title: "36-Month Warranty", desc: "Enjoy extended protection with a 3-year manufacturer warranty." },
                    { title: "Industry-Proven Reliability", desc: "10+ years of service life in real-world deployments." },
                    { title: "Russian Engineering", desc: "Produced in Russia. Delivered to you — without third-party layers." },
                  ].map((item, idx) => (
                    <div key={idx} className="bg-[#F8FAFC] p-8 md:p-10 rounded-[2.5rem] shadow-xl shadow-brand-navy/5 border border-white group hover:-translate-y-2 transition-all duration-500">
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-full max-w-4xl px-4">
          <div className="bg-brand-navy text-white p-4 md:p-6 rounded-[2rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 border border-white/10 backdrop-blur-md">
            <div className="flex flex-col gap-2">
              <p className="text-sm font-bold">Devices selected for comparison:</p>
              <div className="flex flex-wrap gap-2">
                {selectedForCompare.map((name, i) => (
                  <div key={i} className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full border border-white/10 group/tag">
                    <span className="text-xs font-black tracking-widest">{name}</span>
                    <button 
