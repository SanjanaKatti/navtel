export { default as SoftwareHero } from "./SoftwareHero";
export type { CTAItem } from "./SoftwareHero";
export { default as SoftwareKeyFeatures } from "./SoftwareKeyFeatures";
export type { SoftwareFeature } from "./SoftwareKeyFeatures";
export { default as SoftwareContentBlock } from "./SoftwareContentBlock";
export type { ContentBlockItem } from "./SoftwareContentBlock";
export { default as SoftwareCTA } from "./SoftwareCTA";
